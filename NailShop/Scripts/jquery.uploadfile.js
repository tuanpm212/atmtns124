...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        this.hide();
        if (s.messageWrapper) {
            this.errorLog = s.messageWrapper;
        } else {
            this.errorLog = $("<div class='err'></div>"); //Writing errors
        ...
        function setDragDropHandlers(obj, s, ddObj) {
            ...
            ddObj.on('drop', function (e) {
                ...
                if (!s.multiple && files.length > 1) {
                    if (s.showError) {
                        $("<div " + s.styleErr + ">" + s.multiDragErrorStr + "</div>").appendTo(obj.errorLog);
                    }
                    return;
                }
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for (var i = 0; i < files.length; i++) {
                if (!isFileTypeAllowed(obj, s, files[i].name)) {
                    if (s.showError) $("<div " + s.styleErr + "><b>" + files[i].name + "</b> " + s.extErrorStr + s.allowedTypes + "</div>").appendTo(obj.errorLog);
                    continue;
                }
                if (s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if (s.showError) $("<div " + s.styleErr + "><b>" + files[i].name + "</b> " + s.sizeErrorStr + getSizeStr(s.maxFileSize) + "</div>").appendTo(
                        obj.errorLog);
                    continue;
                }
                if (s.maxFileCount != -1 && obj.selectedFiles >= s.maxFileCount) {
                    if (s.showError) $("<div " + s.styleErr + "><b>" + files[i].name + "</b> " + s.maxFileCountErrorStr + s.maxFileCount + "</div>").appendTo(
                        obj.errorLog);
                    continue;
                }
                obj.selectedFiles++;
            ...
            fileInput.change(function () {
                ...
                if (this.files) //support reading files
                {
                    ...
                    if (!isFileTypeAllowed(obj, s, filenameStr)) {
                        if (s.showError) $("<div " + s.styleErr + "><b>" + filenameStr + "</b> " + s.extErrorStr + s.allowedTypes + "</div>").appendTo(
                            obj.errorLog);
                        return;
                    }
                ...
                if (feature.fileapi && feature.formdata) //use HTML5 support and split file submission
                {
                    ...
                    if (s.maxFileCount != -1 && (obj.selectedFiles + fileArray.length) > s.maxFileCount) {
                        if (s.showError) $("<div " + s.styleErr + "><b>" + fileList + "</b> " + s.maxFileCountErrorStr + s.maxFileCount + "</div>").appendTo(
                            obj.errorLog);
                        return;
                    }
        ...
        function ajaxFormSubmit(form, s, pd, fileArray, obj) {
            ...
            var options = {
                ...
                beforeSubmit: function (formData, $form, options) {
                    ...
                    pd.statusbar.append("<div " + s.styleErr + ">" + s.uploadErrorStr + "</div>");
                    pd.cancel.show()
                    form.remove();
                ...
                error: function (xhr, status, errMsg) {
                    ...
                    if (xhr.statusText == "abort") //we aborted it
                    {
                        ...
                        if (s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            pd.statusbar.append("<span " + s.styleErr + ">ERROR: " + errMsg + "</span>");
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
